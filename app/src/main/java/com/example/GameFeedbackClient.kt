...
/**
 * Receives XInput/ViGEm rumble feedback relayed by PC Wheel Receiver.
 *
 * This intentionally uses a dedicated UDP port so controller input and RTT telemetry stay on
 * their existing hot path without extra parsing, buffering or scheduling work.
 */
class GameFeedbackClient(context: Context) : AutoCloseable {
        ...
        try {
            while (currentCoroutineContext().isActive && socket === feedbackSocket) {
                try {
                    ...
                    // ViGEm reports rumble when its value changes, not necessarily every frame.
                    // Refresh a short pulse while the last requested motor state remains non-zero.
    ...
    private fun refreshRumble(force: Boolean) {
        ...
        // Phones usually expose one haptic actuator, while XInput has two rumble motors.
        // Heavy impacts dominate; the small motor still contributes high-frequency texture.
